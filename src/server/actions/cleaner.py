...
IDEOGRAPHIC_VS_MARKERS: Dict[str, str] = {
    chr(
        i
    ): f"Ideographic Variation Selector-{17 + (i - 0xE0100)} (VS{17 + (i - 0xE0100)}, U+{i:05X})"
...
class SimpleLogger:
    ...
    def __init__(
        ...
        self.file_handler = open(log_file, "w", encoding="utf-8") if log_file else None
    ...
    def _detect_file_encoding(self, path: str, fallback: str = "utf-8") -> str:
        ...
        for enc in ["utf-8", "utf-16", "latin-1", "cp1252", sys.getdefaultencoding()]:
            try:
                ...
                continue
        self.log.debug(
            "Could not detect encoding for %s, using fallback: %s", path, fallback
        )
        return fallback
        ...
        for idx, ch in enumerate(text):
            ...
            if ch == "\ufe0f":
                if idx > 0:
                    if emoji_library_available:
                        sequence_to_check = text[
                            idx - 1 : idx + 1
                        ]  # The potential emoji sequence
                        if emoji.is_emoji(sequence_to_check):
                            print(
                                f"Emoji detected: {sequence_to_check} at index {idx} in line {line_num}"
                            )
                            continue
            ...
            if ch in MARKER_CHARS_HIDDEN:
                ...
                if not (is_bom and not self.clean_file):
                    action = (
                        "Processed (BOM)"
                        if is_bom and self.clean_file
                        else ("Removed" if self.clean_file else "Detected")
                    )
        ...
        try:
            ...
            if self.clean_file:
                try:
                    ...
                    out_f = temp_obj
                    self.log.debug(
                        "Created temporary file %s for %s", temp_path, filepath
                    )
            ...
            if temp_path and not content_changed:
                try:
                    ...
                    temp_path = None
                    self.log.debug(
                        "Removed unused temporary file for %s as no changes were made",
                        filepath,
                    )

                except OSError as e_temp_rm:
                    self.log.debug(
                        "Could not remove temporary file %s: %s", temp_path, e_temp_rm
                    )
            ...
            return FileProcessResult(
                filepath, False, None, f"Unexpected error: {e_file_proc}"
            )
    ...
    def _log_file_report(self, filepath: str, line_reports):
        ...
        for ln, (orig, mod, reps) in sorted(line_reports.items()):
            ...
            if mod != orig:
                self.log.info(
                    "  %s: %s %s",
                    self.log.cyan(f"L{ln}"),
                    self.log.cyan("Modified:"),
                    mod.rstrip(),
                )
            for rep in reps:
                action_col = (
        ...
        *,
        ...
        if file_path:
            ...
            if self._is_likely_text_file(file_path):
                ...
                if self.multiple_files:
                    self.log.info(
                        "Skipping '%s' as it does not appear to be text.", file_path
                    )
            return files
        ...
        if dir_path:
            if not os.path.isdir(dir_path):
                ...
                return []
            self.log.debug(
                "Processing directory: %s (recursive=%s)", dir_path, recursive
            )
            walker = (
                os.walk(dir_path)
                if recursive
                else [(dir_path, [], os.listdir(dir_path))]
            )
    ...
    for token in vals:
        try:
            ...
            if parsed:
                out.add(parsed)
                logger.debug(
                    f"Excluding character: '{repr(parsed)}' (U+{ord(parsed):04X})"
                )
            else:
                logger.error(
                    "\n%s for --exclude-char '%s'. Use U+XXXX, plain char, or hex.",
                    logger.red("Error: Invalid format"),
                    token,
                )
                raise SystemExit(1)
    return out
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    hidden_marker_examples = "\n".join(
        [f"  '{m}' : {d}" for m, d in list(HIDDEN_MARKERS.items())[:8]]
    )
    ...
    typo_replacement_examples = "\n".join(
        [
            f"  '{k}' -> '{v}' ({TYPOGRAPHIC_MARKERS.get(k, 'N/A')} to {TYPOGRAPHIC_MARKERS.get(v, 'N/A')})"
            for k, v in TYPOGRAPHIC_REPLACEMENTS.items()
        ]
    )
    if not typo_replacement_examples:
        typo_replacement_examples = "  (No rules defined in TYPOGRAPHIC_REPLACEMENTS)"
    ivs_example_start_char, ivs_example_start_desc = (
        list(IDEOGRAPHIC_VS_MARKERS.items())[0]
        if IDEOGRAPHIC_VS_MARKERS
        else ("U+E0100", "IVS-17")
    )
    ivs_example_end_char, ivs_example_end_desc = (
        list(IDEOGRAPHIC_VS_MARKERS.items())[-1]
        if IDEOGRAPHIC_VS_MARKERS
        else ("U+E01EF", "IVS-256")
    )
    word_chars_examples = "\n".join(
        [
            f"  '{ch}' : {desc}"
            for ch, desc in enumerate(itertools.islice(WORD_COMMON_CHARS, 10))
        ]
    )
...
""",
    ...
    input_group.add_argument(
        "--stdin", action="store_true", help="Read from standard input."
    )
    ...
    parser.add_argument(
        "--exclude-char",
        action="append",
        dest="excluded_chars_str",
        default=[],
        metavar="U+XXXX or Char",
    ...
    parser.add_argument(
        "--report-file", metavar="FILE", help="Write detailed report to a file."
    )
    ...
    parser.add_argument(
        ...
        default="normal",
        help="Set the level of reporting detail.",
    )
    ...
    parser.add_argument(
        "--log-file", metavar="FILE", help="Write logs to a file instead of stdout."
    )
...
def main(argv: Optional[List[str]] = None) -> None:  # noqa: C901 (long, but straight line)
    ...
    if args.clean and changed_paths:
        ...
        if not proceed:
            try:
                ...
                proceed = "yes" # default to yes for our usecase
        ...
        try:
            with open(args.report_file, "w", encoding="utf-8") as f:
                ...
                if stats.total_markers_processed > 0:
                    f.write(
                        f"Total markers processed: {stats.total_markers_processed}\n"
                    )
                f.write(f"Elapsed time: {stats.elapsed_time:.2f} seconds\n\n")

                f.write(
                    f"Status: {'MARKERS FOUND' if stats.files_with_markers > 0 else 'NO MARKERS FOUND'}\n"
                )
    ...
    if args.fail and stats.files_with_markers > 0:
        log.warning(
            "Exiting with status 1 due to --fail flag and detected markers/changes."
        )
        exit_code = 1
